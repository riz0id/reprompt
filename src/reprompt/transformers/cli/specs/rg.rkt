...
(define rg-cli
  (cli:cmd 'rg
     ...
     (cli:flag 'glob 'glob #:aliases '(-g --glob) #:repeat 'list)
     (cli:flag 'iglob 'glob #:aliases '(--iglob) #:repeat 'list)
     ...
     (cli:flag 'threads 'nat #:aliases '(-j --threads))
     (cli:flag 'max-depth 'nat #:aliases '(--max-depth))
